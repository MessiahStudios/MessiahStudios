...
var handleSpecialFiveInteraction = function(square1, square2) {
    ...
    var update = function()
    {
        ...
        if(check.length <= 0) // No blocks need checking
        {
            if(!wholeBoardCheck)
            {
                ...
                if(!validMoves())
                {
                    var sprite = new TextSprite("NO MOVES LEFT!",self.cellSize.x/1.5 + 'px ArtDept1', '#44bce8', 'center', self.topLayer);
    ...
    var pointsPopup = function(position, score)
    {
        ...
        var text = new TextSprite(score,self.cellSize.x/2.4 + 'px ArtDept1', '#44bce8', 'center', self.topLayer);
